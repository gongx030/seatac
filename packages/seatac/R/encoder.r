...
#' encoder_model_vae_position_fragment_size_cnn_kmeans
#'
encoder_model_vae_position_fragment_size_cnn_kmeans <- function(
	latent_dim, 
	n_components,
	fragment_size_filters = c(32L, 32L, 32L),
	fragment_size_kernel_sizes = c(3L, 3L, 3L),
	...
	keras_model_custom(name = name, function(self){
		...
		self$merge_dense_2 <- layer_dense(units = n_components, activation = 'softmax')
		...
		function(x, mask = NULL, training = TRUE){
			...
			list(
				latent = tfd_multivariate_normal_diag(
					loc = y[, 1:latent_dim],
					scale_diag = tf$nn$softplus(y[, (latent_dim + 1):(2 * latent_dim)] + 1e-5)
				),
			  mixture = theta
