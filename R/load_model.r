...
setMethod(
	...
	function(
		...
		z <- tf$random$uniform(shape(1L, model@model$vae$latent_dim))
		sequence <- tf$zeros(shape(1L, model@model$vae$block_size))
		res <- model@model(list(z = z, sequence = sequence))
		load_model_weights_tf(model@model, filename)
		model
	}
)
...
setMethod(
	...
	function(
		...
		sequence <- tf$zeros(shape(1L, model@model$vae$block_size))
		res <- model@model(list(sequence = sequence))
		load_model_weights_tf(model@model, filename)
		model
	}
)
