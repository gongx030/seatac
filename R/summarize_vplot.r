...
setMethod(
	...
	function(
		...
		x <- x[rowSums(assays(x)$counts) > 0]

		classes <- names(annotation)
		annotation <- unlist(annotation)
		labels <- as.numeric(factor(names(annotation), classes))
		...
		counts <- tf$zeros(shape(length(classes), n_bins_per_block * x@n_intervals))
		...
		se <- SummarizedExperiment(
			...
			rowData = data.frame(freq = as.numeric(freq), class = classes)
		)
