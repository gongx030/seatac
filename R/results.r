setMethod(
	...
	function(x, targets, batch_size = 512L, B = 100L, ...){

		blocks <- extract_blocks(x)
		centers <- granges(blocks) %>%
			resize(fix = 'center', width = x@bin_size)

		pvalues <- lapply(targets, function(tgt){
			tgt <- resize(tgt, fix = 'center', width = 1L)
			z <- blocks$latent[centers %over% tgt, ]
			ht <- hotelling.test(z, blocks$latent[sample(1:length(blocks), B), ])
			ht$pval
		}) %>% unlist()
