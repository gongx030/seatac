...
setMethod(
	...
	function(
		...
		ws <- tf$cast(model@window_size / 2, tf$float32)
		...
		for (epoch in seq_len(epochs)){
			...
			for (i in seq_len(n_batch)){
				...
				# read counts for each data point (non-zero pixels)
				m <- xc@vals %>% 
					tf$cast(tf$float32) %>%
				...
				with(tf$GradientTape(persistent = TRUE) %as% tape, {
					...
					theta <- z %>% 	# batch size ~ 1
						model@mixture()

					theta <- tf$tensordot(g, theta, 1L)
					...
					nl <- theta * tfd_normal(y_pred, model@sigma0)$log_prob(y) + (1 - theta) * tfd_uniform(low = 0, high = model@window_size / 2)$log_prob(y)

					loss_likelihood <- (-w * m * nl) %>%
						tf$reduce_mean()
...
#				mixture_gradients <- tape$gradient(loss, model@mixture$trainable_variables)
#				list(mixture_gradients, model@mixture$trainable_variables) %>%
#					purrr::transpose() %>%
#					optimizer$apply_gradients()
			}
